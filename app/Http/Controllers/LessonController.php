...
class LessonController extends Controller
{
    ...
    /**
     * Serve video through a signed proxy — hides the real video source from students.
     * For locally hosted files: streams directly from storage.
     * For external URLs (YouTube, Vimeo, Drive): validates enrollment and returns
     * the embed URL only as JSON, never in page source.
     */
    public function serveVideo(Lesson $lesson)
    {
        $user = Auth::user();
        if (!$user) {
            abort(403);
        ...
        // Must be enrolled, or be the instructor/admin
        $isInstructor = $user->id === $course->instructor_id;
        $isAdmin = $user->role === 'admin';
        $isEnrolled = $user->enrolledIn($course->id);
        ...
        // ---- LOCAL / UPLOADED VIDEO: stream it server-side ----
        // If the URL is a relative path or points to our own storage
        if (!str_starts_with($rawUrl, 'http') || str_contains($rawUrl, config('app.url'))) {
            // Strip the app URL prefix if present
            $relativePath = str_replace(config('app.url') . '/', '', $rawUrl);
            ...
            if ($range) {
                // Range request — support seeking in video
        ...
        // ---- EXTERNAL EMBED (YouTube / Vimeo / Drive): return safe embed URL as JSON ----
        $embedUrl = null;
        if (preg_match('/drive\.google\.com\/(?:file\/d\/|open\?id=)([^\/\?&]+)/i', $rawUrl, $m)) {
            $embedUrl = 'https://drive.google.com/file/d/' . $m[1] . '/preview';
        } elseif (preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i', $rawUrl, $m)) {
            $embedUrl = 'https://www.youtube.com/embed/' . $m[1] . '?rel=0&modestbranding=1&disablekb=0';
        } elseif (preg_match('/vimeo\.com\/(\d+)/i', $rawUrl, $m)) {
            $embedUrl = 'https://player.vimeo.com/video/' . $m[1] . '?dnt=1';
        } else {
            $embedUrl = $rawUrl; // Passthrough for other embeds
        }

        return response()->json(['embed' => $embedUrl]);
    }
}
