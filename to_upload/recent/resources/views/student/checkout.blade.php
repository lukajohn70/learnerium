        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-1">
                <div class="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden sticky top-24">
                    ...
                    {{-- Preorder Notice --}}
                    @if($isPreorder)
                    <div class="bg-purple-600 text-white px-6 py-3 flex items-center gap-3">
                        <i class="fas fa-bookmark text-lg flex-shrink-0"></i>
                        <div>
                            <p class="font-extrabold text-sm">You are Pre-Ordering this Course</p>
                            <p class="text-purple-200 text-xs">You'll get full access as soon as the course officially launches.</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex justify-between text-sm text-gray-500">
                            <span>{{ $isPreorder ? 'Pre-Order Price' : 'Original Price' }}</span>
                            <span class="font-bold {{ $isPreorder ? 'text-purple-700' : 'text-gray-800' }}" id="original_price_display">₦{{ number_format($course->effectivePrice(), 2) }}</span>
                        </div>
                        @if($isPreorder && $course->price > 0)
                        <div class="flex justify-between text-xs text-gray-400">
                            <span>Launch price</span>
                            <span class="line-through">₦{{ number_format($course->price, 2) }}</span>
                        </div>
                        @endif
                        ...
                        <div class="flex justify-between items-baseline text-base text-gray-900 font-extrabold">
                            ...
                            <span class="{{ $isPreorder ? 'text-purple-700' : 'text-primary-jlm' }} text-xl" id="total_val">₦{{ number_format($course->effectivePrice(), 2) }}</span>
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let basePriceNgn = {{ (float) $course->effectivePrice() }};
